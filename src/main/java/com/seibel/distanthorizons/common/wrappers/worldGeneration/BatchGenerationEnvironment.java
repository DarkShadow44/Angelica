...
public final class BatchGenerationEnvironment extends AbstractBatchGenerationEnvironmentWrapper
{
    ...
    private CompletableFuture<Chunk> forceLoadChunkAsync(WorldServer level, int x, int z) {
        return ForgeServerProxy.schedule(true, () ->
            {
                long chunk = ChunkCoordIntPair.chunkXZ2Int(x, z);
    ...
    private CompletableFuture<Void> releaseChunkToServer(WorldServer level, ChunkPos pos, boolean chunkWasGeneratedUpToFeatures)
    {
        ...
        {
            long chunk = ChunkCoordIntPair.chunkXZ2Int(pos.x, pos.z);
